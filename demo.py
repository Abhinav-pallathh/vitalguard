...
def show(pb, scenario, seconds=25.0, live=False):
    ...
    print(f"  {'time':>6}  {'':6}  {'reading':<34} why")
    print(f"  {'-'*6}  {'-'*6}  {'-'*34} {'-'*20}")
    for w in windows(synth.generate(scenario, duration_s=seconds, seed=11)):
        ...
        if d is not None:
            reading = f"{d.hr_bpm:5.0f} bpm   {d.delta_bpm:+5.0f} vs {d.baseline_bpm:.0f}   {d.personal_sigma:+5.1f}sd"
        elif not v.scored:
            reading = "  -- no reading --"
        else:
            reading = "  -- calibrating --"
        ...
        print(f"  {w.t_end_ms/1000:5.1f}s  {MARK[v.ppg]}  {reading:<34} {why}")
        if live:
            time.sleep(0.12)
    print()
...
def main():
    ...
    print(BAR)
    print("Every number above passed the quality gate before it was compared to")
    print("the baseline. The rows with no reading are the product working.")
    print(BAR)
